...
public class PedidosService {
	...
	public void eliminarPedido(String producto) {
		try {
			List<String> ListaActualizada = Files.lines(pt).
					map(Util::convertirCadenaAPedido).filter(p-> !p.getProducto().equals(producto)).
					map(Util::convertirPedidoACadena).toList();
             Files.write(pt,ListaActualizada);
		}catch(IOException e) {
			e.printStackTrace();
		}
		...
		try {
			return Files.lines(pt) 
					.map(p ->{
						...
						return new Pedido(nombre, unidades, fecha);
                    })
